...
[data-testid="stSidebar"] .stButton > button:hover { background-color: #ff2d78 !important; border-radius: 20px !important; }
...
div[data-testid="stTextInput"] input { background-color: #1e1e1e !important; color: white !important;
    border: 1px solid #333 !important; border-radius: 10px !important; font-size: 14px !important; padding: 14px 18px !important; }
...
if st.session_state.page == "home":
    ...
    st.markdown(f"""
    <div class="hero-container">{img_tag}<div class="hero-overlay"></div>
    ...
    with col_input:
        url = st.text_input("url", placeholder="https://www.google.com/maps/search/cornell+tech+cafe", label_visibility="collapsed", key="url_field")
    with col_btn:
    ...
    div[data-testid="stMainBlockContainer"] .stButton > button {
        background-color: #ff2d78 !important; color: white !important; border: none !important;
    ...
    <div class="cards-section">
        <div class="card card-green"><h3>Zero Setup</h3><p>Just paste your Google Maps link. The model reads your venue data and does the rest automatically.</p></div>
        <div class="card card-teal"><h3>Curated for You!</h3><p>Music adapts to your venue's atmosphere, using ML to predict the perfect audio profile from real venue data.</p></div>
    </div>""", unsafe_allow_html=True)
    ...
    <div class="venue-header"><h1>{venue_name}</h1><p>{subtitle}</p></div>""", unsafe_allow_html=True)
    ...
    with left_col:
        ...
        st.markdown(f"""
        <div class="box"><div class="box-title">Vibe Tags</div><div class="vibe-grid">{vibe_pills}</div></div>
        <div class="box"><div class="box-title">Audio Profile Breakdown</div>{sentiment_html}
            <div class="legend">
    ...
    <style>
    div[data-testid="stMainBlockContainer"] .stButton > button {
        background-color: #00bcd4 !important; color: white !important; font-size: 18px !important;
        font-weight: 700 !important; border-radius: 14px !important; padding: 18px !important;
        letter-spacing: 1px !important; border: none !important; position: relative !important; height: auto !important;
    }
    </style>""", unsafe_allow_html=True)
    ...
    div[data-testid="stMainBlockContainer"] .stButton > button {
        background-color: transparent !important; color: white !important; border: none !important;
        font-size: 16px !important; font-weight: 600 !important; padding: 8px 12px !important;
        position: relative !important; height: auto !important; border-radius: 8px !important;
    }
    div[data-testid="stMainBlockContainer"] .stButton > button:hover { background-color: #ff2d78 !important; }
    </style>
    ...
    <div class="playlist-banner">
        <div class="album-art"><div class="album-art-label">ATMO<br>SOUND</div><div style="font-size:28px;">🎵</div></div>
    ...
    if playlist_df.empty:
        ...
        for i, row in enumerate(playlist_df.itertuples(), start=1):
            ...
            if track_id and str(track_id) != "nan":
                st.markdown(
                    f'<iframe src="https://open.spotify.com/embed/track/{track_id}?utm_source=generator&theme=0" '
                    f'width="100%" height="80" frameborder="0" allowtransparency="true" '
                    f'allow="encrypted-media" style="border-radius:8px;margin:-8px 0 4px 0;"></iframe>',
                    unsafe_allow_html=True
                )
